...
function renderInformeContenido(){
  ...
  document.getElementById('informe-content').innerHTML = `
    ...
    <div class="card" style="margin-bottom:1.2rem;">
      <h3>Por categoría</h3>
      <div style="position:relative; height:260px; margin-top:0.5rem;"><canvas id="chart-informe-categoria"></canvas></div>
    </div>

    <div class="card">
      <h3>Detalle completo</h3>
      ${informeScope === 'anio'
        ? Object.entries(agrupar(items, g => mesDeFecha(g))).map(([mes, its]) => acordeonGrupo(mes, its, null, {})).join('')
        : tablaItems(items)}
    </div>
  `;

  if (informeScope === 'anio') activarAcordeones();
  ...
  if (chartDisponible()) {
    ...
    chartInforme = new Chart(ctxInforme, {
      type: 'doughnut',
      data: { labels: catEntradas.map(([n]) => n), datasets: [{ data: catEntradas.map(([,t]) => t), backgroundColor: COLORES, borderColor: '#fff', borderWidth: 2 }] },
      options: { plugins: { legend: { position: 'bottom', labels: { boxWidth: 8, font: { size: 10 } } },
    ...
    ctxInforme.parentElement.innerHTML = '<p style="color:var(--dim); font-size:0.85rem;">Gráfico no disponible (Chart.js no cargó).</p>';
  }
}
